...
public class TestVoyageRepository {
	...
	@WeldSetup
	private WeldInitiator weld = WeldInitiator.from(VoyageRepository.class, VoyageurDAO.class, VoyageDAO.class,
	...
	void testGetVoyagesComposedByAtLeastOneTrajetOf() {
		...
		trajetsV1.add(trajet1);
		trajetsV1.add(trajet2);
		trajetsV1.add(trajet3);
		List<Trajet> trajetsV2 = new ArrayList<Trajet>();
		trajetsV2.add(trajet1);
		trajetsV2.add(trajet4);
		trajetsV2.add(trajet5);
		...
		List<Voyage> v = voyageRepository.getVoyagesComposedByAtLeastOneTrajetOf(trajets);
		assertEquals(1, v.size());
		trajets.add(trajet1);
		assertEquals(2, voyageRepository.getVoyagesComposedByAtLeastOneTrajetOf(trajets).size());
	}
