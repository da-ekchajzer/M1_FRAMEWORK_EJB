...
public class GatewayInfocentre {
	...
	public static ItineraireJAXB getItineraire(int idTrain) {
		...
		Response resp = target.path("itineraire").path("" + idTrain).request().accept(MediaType.APPLICATION_XML).get();
	...
	public static void sendCurrenArret(ArretJAXB arretJAXB, int idTrain) {
		WebTarget target = client.target("http://localhost:8080");
		Response resp = target.path("itineraire").path("" + idTrain).request().post(Entity.xml(arretJAXB));
	}
	
	public static void updateIncident(int etatIncident, int idTrain) {
		WebTarget target = client.target("http://localhost:8080");
		Response resp = target.path("incident").path("" + idTrain).path("" + etatIncident).request().put(Entity.xml(null));
	}
	
	public static void sendIncident(IncidentJAXB incidentJAXB, int idTrain) {
		WebTarget target = client.target("http://localhost:8080");
		Response resp = target.path("incident").path("" + idTrain).request().post(Entity.xml(incidentJAXB));
	}
